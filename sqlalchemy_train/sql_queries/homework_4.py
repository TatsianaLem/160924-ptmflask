...
# Задание 1: Добавление категорий и продуктов
categories = [
...
# Задание 2: Чтение данных с использованием конструкции with
with Session() as session:
    # Извлечение всех категорий
    categories = session.query(Category).all()

    # Вывод категорий и связанных с ними продуктов
    for category in categories:
        print(f"Категория: {category.name} - {category.description}")
        for product in category.products:
            print(f"  Продукт: {product.name}, Цена: {product.price}")

# Задание 3: Обновление данных с использованием конструкции with
with Session() as session:
    # Найти продукт "Смартфон" и обновить его цену
...
# Задание 4: Агрегация и группировка с использованием конструкции with
with Session() as session:
    # Подсчет общего количества продуктов в каждой категории
...
# Задание 5: Группировка с фильтрацией с использованием конструкции with
with Session() as session:
    # Отфильтровываем категории, в которых более одного продукта
